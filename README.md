...
- **主辦專區**：活動新增/編輯/刪除（含貼上海報圖片網址）、核對匯款、審核取消申請、強制取消單一會員報名、會員總覽、行前通知都在這一頁。
...
4. **主辦方強制取消**：「主辦專區」的「報名名單／強制取消」選一個活動，會列出目前有效的報名者（名字、Email、報名時間、繳費與報到狀態）。每一位旁邊有「強制取消」按鈕，不用等會員申請，主辦方可以直接取消單一會員的報名（例如重複報名、資格不符、需要調整名額）（`js/events.js` 的 `adminForceCancelTicket`）：
   - 有繳費或已送出匯款通知的票，可以選退款比例（全額／50%／不退），預設全額，實際匯款退還仍由主辦方自己處理；免費或未繳費的票退款比例一律 0%。
   - 可以填取消原因，會員在「我的票券」會看到「已由主辦單位取消」和原因。
   - 釋出名額（`currentCount` -1）；不會算進會員自己的「取消次數」；報名時用掉的 95 折折扣券會還給會員；如果會員剛好有審核中的取消申請，會一併結案。
   - 已報到的會員不能強制取消。取消後會員之後仍可以重新報名。
   - 不需要改 Firestore 規則（主辦方本來就能更新票券、活動、折扣券）。
